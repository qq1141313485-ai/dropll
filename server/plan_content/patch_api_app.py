...
IMPORT_LINE = "from plans import install_plan_routes"
APP_ASSIGNMENT_RE = re.compile(r"^\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*FastAPI\s*\(")
INSTALL_CALL_RE = re.compile(r"\binstall_plan_routes\s*\(")
...
def patch_text(source: str) -> tuple[str, bool]:
    ...
    if any(IMPORT_LINE in line for line in lines) and any(INSTALL_CALL_RE.search(line) for line in lines):
        return source, False

    if not any(IMPORT_LINE in line for line in lines):
        import_pos = _find_insert_position(lines)
    ...
    if not any(INSTALL_CALL_RE.search(line) for line in lines):
        ...
        call_pos = _find_call_position(lines, app_index)
        lines.insert(call_pos, f"install_plan_routes({app_name})")
        changed = True
...
def patch_file(path: Path, *, dry_run: bool = False) -> bool:
    ...
    if dry_run:
        ...
        return True
    backup = path.with_name(f"{path.name}.bak-plan-routes-{time.strftime('%Y%m%d-%H%M%S')}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Patch FastAPI app.py to install plan routes.")
